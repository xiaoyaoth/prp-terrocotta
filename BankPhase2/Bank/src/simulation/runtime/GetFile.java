...
public class GetFile extends Thread {
	...
	public static void main(String args[]) {
		...
		try {
			//getFile = new GetFile(10000,"agentsIn//");
			getFile = new GetFile(10001, "snrQueue\\");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public GetFile(int port, String dirName) {
		try {
	...
	public void run() {
		while (true) {
			try {
				...
				new Thread(new DealWithReq(tempSocket, this.dirName)).start();
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
...
final class DealWithReq implements Runnable {
	...
	public DealWithReq(Socket s, String dirName) throws IOException {
		this.tempSocket = s;
		this.dirName = dirName;
	}
	...
	public void run() {
		File f;
		try {
			f = new File(this.dirName + filecounter++);
